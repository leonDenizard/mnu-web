...
export function ProductAccordion({ product, isRemoving, onRemove }: ProductAccordionProps) {
  ...
  const linkMutation = useMutation({ mutationFn: (modifierGroupId: string) => linkModifierGroupToProduct(product.id, modifierGroupId, accessToken!), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['product-modifier-groups', product.id] }); setFeedback({ kind: 'success', message: 'Adicional vinculado ao produto.' }) }, onError: () => setFeedback({ kind: 'error', message: 'Não foi possível vincular o adicional.' }) })
  const unlinkMutation = useMutation({ mutationFn: (modifierGroupId: string) => unlinkModifierGroupFromProduct(product.id, modifierGroupId, accessToken!), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['product-modifier-groups', product.id] }); setFeedback({ kind: 'success', message: 'Adicional removido do produto.' }) }, onError: () => setFeedback({ kind: 'error', message: 'Não foi possível remover o adicional.' }) })
  ...
  return <><SortableProduct product={product} isRemoving={isRemoving} onRemove={onRemove} onEdit={openEditDialog} header={<button className="flex min-w-0 flex-1 items-center justify-between gap-3 text-left" type="button" onClick={toggleProduct} disabled={!canExpand || modifierGroupsQuery.isFetching} aria-expanded={isExpanded}>
    ...
    </div>}</SortableProduct>{isEditDialogOpen && <ProductEditDialog name={name} price={price} image={image} groups={groups} availableGroups={availableGroupsQuery.data?.data ?? []} selectionLimits={selectionLimits} isSaving={updateProductMutation.isPending} isLoadingGroups={availableGroupsQuery.isPending || linkMutation.isPending || unlinkMutation.isPending} onClose={() => setIsEditDialogOpen(false)} onNameChange={setName} onPriceChange={setPrice} onImageFileChange={changeImage} onLimitChange={(groupId, field, value) => setSelectionLimits((current) => ({ ...current, [groupId]: { ...(current[groupId] ?? groups.find((group) => group.id === groupId)!), [field]: Math.max(0, Number(value)) } }))} onToggleGroup={(groupId, linked) => linked ? unlinkMutation.mutate(groupId) : linkMutation.mutate(groupId)} onSave={() => updateProductMutation.mutate()} />}{feedback && <FeedbackToast feedback={feedback} onClose={() => setFeedback(null)} />}</>
}
...
function ProductEditDialog({ name, price, image, groups, availableGroups, selectionLimits, isSaving, isLoadingGroups, onClose, onNameChange, onPriceChange, onImageFileChange, onLimitChange, onToggleGroup, onSave }: ProductEditDialogProps) {
