...
export function getProducts(categoryId: string, accessToken: string) {
  return apiRequest(`/api/menu/products/${categoryId}?limit=50`, productsResponseSchema, {
    headers: { Authorization: `Bearer ${accessToken}` },
  })
}

export function createProduct(input: ProductInput, accessToken: string) {
  return apiRequest('/api/menu/products', productResponseSchema, {
    method: 'POST',
...
export function deleteProduct(id: string, accessToken: string) {
  return apiRequest(`/api/menu/products/${id}`, productResponseSchema, {
    method: 'DELETE',
    headers: { Authorization: `Bearer ${accessToken}` },
  })
}
...
export function reorderProducts(products: Product[], accessToken: string) {
  return Promise.all(
    products.map((product, displayOrder) =>
      updateProduct(
        product.id,
        {
          name: product.name,
